#!/bin/bash
#SBATCH --job-name=pretrain_subsample_sent_len       # 任务名
...
#SBATCH --output=./logs/pretrain/%x%A.out           # 正常输出写入的文件
#SBATCH --error=./logs/pretrain/%x%A.err            # 报错信息写入的文件
